...
**Status:** pending
**aibox CLI:** v0.33.1 → v0.34.1
**processkit:** v0.28.6

## Summary

aibox has been updated from v0.33.1 to v0.34.1. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.33.1 and v0.34.1 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.34.1 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.34.1` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.34.1"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.34.1.
