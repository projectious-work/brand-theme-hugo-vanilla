...
**Status:** pending
**aibox CLI:** v0.34.6 → v0.34.7
**processkit:** v0.28.8

## Summary

aibox has been updated from v0.34.6 to v0.34.7. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.34.6 and v0.34.7 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.34.7** (processkit v0.28.8): Patch release: adds persistent Yazi line-number and wrapping controls, explicit preview scrolling, and tabular recursive directory-size reports with visible hierarchy totals.
  <https://github.com/projectious-work/aibox/releases/tag/v0.34.7>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.34.7 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.34.7` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.34.7"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.34.7.
