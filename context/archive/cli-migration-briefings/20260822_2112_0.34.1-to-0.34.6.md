...
**Status:** pending
**aibox CLI:** v0.34.1 → v0.34.6
**processkit:** v0.28.8

## Summary

aibox has been updated from v0.34.1 to v0.34.6. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.34.1 and v0.34.6 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- **v0.34.3** (processkit v0.28.8): Patch release: restores readable Yazi marked items across every theme, repairs PowerKit separator colors, preserves active-pane emphasis, uses supported Codex lifecycle hooks, and reliably restores the outer terminal title.
  <https://github.com/projectious-work/aibox/releases/tag/v0.34.3>
- **v0.34.4** (processkit v0.28.8): Patch release: updates the bundled Yazi pane-toggle plugin to the current indexed ratio API and removes runtime deprecation warnings.
  <https://github.com/projectious-work/aibox/releases/tag/v0.34.4>
- **v0.34.5** (processkit v0.28.8): Patch release: restores PowerKit window-separator color continuity across every theme, makes isolated visual regressions mandatory for releases, and refreshes deferred tool pins.
  <https://github.com/projectious-work/aibox/releases/tag/v0.34.5>
- **v0.34.6** (processkit v0.28.8): Patch release: makes the PowerKit separator compatibility patch accept the expanded pinned renderer while retaining structural format validation, restoring host runtime image builds.
  <https://github.com/projectious-work/aibox/releases/tag/v0.34.6>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.34.6 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.34.6` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.34.6"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.34.6.
